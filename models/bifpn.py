...
class BiFPNModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(levels - 1, 0, -1):
            pathtd[i - 1] = w1[0, kk] * pathtd[i - 1] + w1[
                1, kk] * F.interpolate(
        ...
        for i in range(0, levels - 2, 1):
            pathtd[i + 1] = w2[0, i] * pathtd[i + 1] + w2[1, i] * F.max_pool2d(
                pathtd[i], kernel_size=2) + w2[2, i] * \
                            inputs[i + 1]
            pathtd[i + 1] = self.bifpn_convs[jj](pathtd[i + 1])
            jj = jj + 1

        pathtd[levels - 1] = w1[0, kk] * pathtd[levels - 1] + w1[
            1, kk] * F.max_pool2d(pathtd[levels - 2],
                                  kernel_size=2)
