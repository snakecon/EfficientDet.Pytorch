...
parser.add_argument('--weights',
                    default='./weights/checkpoint_efficientdet-d0_154.pth',
                    type=str,
...
if (args.weights is not None):
    ...
    model = EfficientDet(num_classes=num_class, network=network,
                         is_training=False)
...
def eval_voc(iou_threshold=0.5):
    model.eval()
    with torch.no_grad():
        all_detections = [[None for i in range(valid_dataset.__num_class__())]
                          for j in range(len(valid_dataset))]
        all_annotations = [[None for i in range(valid_dataset.__num_class__())]
                           for j in range(len(valid_dataset))]
        for idx, (images, annotations) in enumerate(tqdm(valid_dataloader)):
            ...
            if (scores.shape[0] > 0):
                ...
                for label in range(valid_dataset.__num_class__()):
                    all_detections[idx][label] = pred_annots[
                                                 pred_annots[:, -1] == label,
                                                 :-1]
            else:
                for label in range(valid_dataset.__num_class__()):
            ...
            for label in range(valid_dataset.__num_class__()):
                all_annotations[idx][label] = annotations[
                                              annotations[:, 4] == label,
                                              :4].copy()
